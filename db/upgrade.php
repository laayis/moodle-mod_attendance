...
function xmldb_attendance_upgrade($oldversion=0) {
    ...
    if ($oldversion < 2017071801) {
        // Define field setunmarked to be added to attendance_statuses.
        $table = new xmldb_table('attendance_warning_done');
        $field = new xmldb_field('sessionid', XMLDB_TYPE_INTEGER, '10', null, true, null, '1', 'notifyid');
        ...
        $index = new xmldb_index('notifyid', XMLDB_INDEX_UNIQUE, array('notifyid', 'userid, sessionid'));
        $dbman->add_index($table, $index);

        // Attendance savepoint reached.
        upgrade_mod_savepoint(true, 2017071801, 'attendance');
    }
    return $result;
}
